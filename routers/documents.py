...
async def _vectorize_knowledge_document_background(
    document_id: int, stored_file_path: str
):
    ...
    async with AsyncSessionLocal() as bg_db:
        try:
            ...
            if not document:
                logger.warning(
                    "knowledge vectorize skipped, document not found, id=%s",
                    document_id,
                )
                return
            ...
            await bg_db.commit()
            logger.info(
                "knowledge document vectorized in background, document_id=%s",
                document_id,
            )
        except Exception as exc:
            await bg_db.rollback()
            logger.exception(
                "knowledge document background vectorize failed, document_id=%s",
                document_id,
            )
            source = await _get_source_document_by_path(bg_db, stored_file_path)
            if source:
    ...
    for file, file_name in zip(file_list.file_list, file_list.file_name):
        ...
        try:
            ...
            if _is_knowledge_library(file_list.library_type):
                ...
                await set_document_tag_names(
                    db, document, file_list.tag, created_by=current_user_id
                )
                ...
                background_tasks.add_task(
                    _vectorize_knowledge_document_background, document.id, file
                )
            ...
            if not _has_meaningful_title(document):
                ...
                await _mark_source_parse_failed(
                    db, file, "AI解析失败：未能生成有效标题。", file_list.library_type
                )
                continue
